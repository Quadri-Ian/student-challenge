...
export default function AdminAnalytics() {
  ...
  const [scoredByJudge, setScoredByJudge] = useState<Set<string>>(new Set());
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const jList: JudgeOption[] = [];
      snap.docs.forEach((d) => {
        const data = d.data();
        if (data.role === 'judge' || data.role === 'admin') {
          jList.push({ uid: d.id, email: data.email });
        }
      });
      setJudges(jList);
    });
  ...
  useEffect(() => {
    if (!judgeFilter) { setScoredByJudge(new Set()); return; }
    const q = collection(db, 'judgeScores', judgeFilter, 'subs');
    const unsub = onSnapshot(q, (snap) => {
      setScoredByJudge(new Set(snap.docs.map((d) => d.id)));
    });
    return () => unsub();
  }, [judgeFilter]);

  const filtered = submissions.filter((s) => {
    if (judgeFilter && !scoredByJudge.has(s.id)) return false;
    ...
    <AdminLayout>
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-2">
          <label className="text-sm">Judge</label>
          <select className="border rounded px-2 py-1 text-sm" value={judgeFilter} onChange={(e) => setJudgeFilter(e.target.value)}>
            <option value="">All</option>
            {judges.map((j) => <option key={j.uid} value={j.uid}>{j.email}</option>)}
          </select>
        </div>
        <div className="flex items-center gap-2">
